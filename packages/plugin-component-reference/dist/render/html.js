...
export function renderComponentReferenceHtml(entry) {
    ...
    const examples = entry.examples?.length
        ? entry.examples
            .map((example, index) => `<h4 class="pp-component-example-title">Example ${index + 1}</h4><pre class="pp-component-example"><code>${escapeHtml(example)}</code></pre>`)
            .join("")
        : "";
    return [
...
export function lookupComponent(manifest, lookup) {
    ...
    if (lookup.exportName) {
        return Object.values(manifest.components).find((entry) => entry.exportName === lookup.exportName &&
