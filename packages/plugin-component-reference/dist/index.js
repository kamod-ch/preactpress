...
import { buildComponentManifest, defaultApiManifestPath } from "./extract/generate.js";
...
export function componentReferencePlugin(options = {}) {
    let manifestPromise;
    let manifestRef;
    const loadManifest = async (root, configDir) => {
        if (!manifestPromise) {
    ...
    return {
        ...
        async config(config) {
            ...
            return {
                ...
                vite: {
                    ...config.vite,
                    plugins: [...(config.vite?.plugins ?? []), vitePlugin],
                },
            };
        },
        async configResolved(config) {
            await loadManifest(config.root, config.configDir);
            config.logger.info(`component-reference: indexed ${Object.keys(manifestRef?.components ?? {}).length} component(s)`, { timestamp: true });
        },
        transformMarkdown(source) {
            if (!manifestRef || !source.includes("<ComponentReference"))
                return undefined;
            const replaced = source.replace(COMPONENT_REFERENCE_RE, (_match, attrSource) => {
        ...
        transformPageData(page, ctx) {
            if (!manifestRef || page.kind !== "markdown")
                return page;
            if (!page.html.includes("pp-component-reference"))
                return page;
            ...
            for (const entry of Object.values(manifestRef.components)) {
                if (page.html.includes(`component-${entry.name}`)) {
                    for (const tag of componentSearchTags(entry))
                        tags.add(tag);
                }
            }
