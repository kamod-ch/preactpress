...
function componentFromSymbol(symbol, manifest) {
    const propsInterface = Object.values(manifest.symbols).find((entry) => entry.name === `${symbol.name}Props` ||
...
export function componentsFromApiManifest(manifest) {
    ...
    for (const symbol of Object.values(manifest.symbols)) {
        const isComponent = symbol.tags?.includes("component") ||
            symbol.kind === "function" ||
            (symbol.kind === "interface" && symbol.name.endsWith("Props"));
        if (!isComponent)
            continue;
        if (symbol.kind === "interface" && symbol.name.endsWith("Props"))
            continue;
