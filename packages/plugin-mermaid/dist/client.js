...
function normalizeMermaidBlocks() {
    const blocks = Array.from(document.querySelectorAll(".pp-mermaid"));
    for (const code of document.querySelectorAll('pre > code.language-mermaid, pre > code[class*="language-mermaid"]')) {
        const pre = code.parentElement;
        if (!pre || pre.dataset.mermaidNormalized === "true")
            continue;
...
function renderErrorMessage(block, source, message) {
...
export async function enhanceContent() {
    if (typeof document === "undefined")
        return;
    const blocks = normalizeMermaidBlocks().filter((block) => block.dataset.mermaidRendered !== "true");
    if (blocks.length === 0)
        return;
    const run = ++renderRun;
