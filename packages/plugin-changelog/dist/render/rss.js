...
function releaseSummary(release) {
    ...
    if (release.description)
        parts.push(release.description.slice(0, 280));
    const counts = {
...
export function renderChangelogAtomFeed(manifest, options) {
    ...
    const entries = manifest.releases
        ...
        .join("\n");
    const updated = manifest.releases.find((release) => release.date)?.date ?? new Date().toISOString();
