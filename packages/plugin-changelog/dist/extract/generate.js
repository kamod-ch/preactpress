...
function buildProviderContext(config, options, cacheDir) {
    return {
        root: config.root,
        cacheDir,
...
async function fetchAllRawReleases(options, context) {
...
function buildReleases(rawReleases, baseRoute) {
...
function versionScopedReleases(releases, docVersion, versionBaseRoute) {
    return releases
...
export async function generateChangelogDocs(config, options) {
...
export async function writeStructuredManifest(configDir, manifest) {
    ...
    await fs.writeFile(path.join(configDir, "changelog-manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
}
