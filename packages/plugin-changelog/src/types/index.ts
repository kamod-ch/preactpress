...
export const CHANGELOG_MANIFEST_VERSION = 1 as const;
...
export class ChangelogRateLimitError extends Error {
  ...
  constructor(
    provider: ChangelogProviderId,
    message: string,
    options?: { resetAt?: string; remaining?: number; cause?: unknown },
  ) {
...
export class ChangelogOfflineError extends Error {
  ...
  constructor(provider: ChangelogProviderId, message: string, options?: { cause?: unknown }) {
    super(message, options);
