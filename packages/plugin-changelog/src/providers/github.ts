import {
  ...
  type RawChangelogRelease,
} from "../types/index.js";
import { readRemoteCache, writeRemoteCache } from "../extract/cache.js";
import { parseReleaseBody } from "../extract/normalize.js";
import type { ChangelogProvider, GitHubReleasePayload, ProviderContext } from "./types.js";

const GITHUB_API = "https://api.github.com";

function resolveToken(context: ProviderContext): string | undefined {
...
function formatRateLimitMessage(remaining: number | undefined, resetAt?: string): string {
  const parts = [
...
function mapGitHubRelease(release: GitHubReleasePayload): RawChangelogRelease {
  const body = release.body ?? "";
  const sections = parseReleaseBody(body);
  const contributors = new Set<string>();
  if (release.author?.login) contributors.add(release.author.login);
...
async function fetchGitHubReleases(
  ...
  if (context.offline) {
    ...
    throw new ChangelogOfflineError(
      "github",
      `Offline build requested but no cached GitHub releases found for ${repository}. Run a online build first.`,
    );
  }
  ...
  try {
    ...
    throw new ChangelogOfflineError(
      "github",
      `Failed to fetch GitHub releases for ${repository}. ${error instanceof Error ? error.message : String(error)}`,
      { cause: error },
    );
  }
...
export const githubChangelogProvider: ChangelogProvider = {
  id: "github",
