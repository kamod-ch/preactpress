...
function renderMeta(symbol: ApiSymbol): string {
  const parts: string[] = [];
...
function renderSignature(signature: ApiSignature, manifest: ApiManifest, route: string): string {
...
function renderMembers(symbol: ApiSymbol, manifest: ApiManifest): string {
  if (!symbol.members?.length) return "";
  const sections: string[] = ["## Members", ""];
...
function renderEnumMembers(symbol: ApiSymbol): string {
  if (!symbol.enumMembers?.length) return "";
  const rows = symbol.enumMembers.map(
    (member) => `| \`${member.name}\` | ${member.value ?? ""} | ${member.description ?? ""} |`,
  );
  return [
    "## Enum members",
    "",
...
function renderSymbolPage(symbol: ApiSymbol, manifest: ApiManifest): string {
...
function renderIndexPage(manifest: ApiManifest): string {
...
export function renderApiPages(manifest: ApiManifest): ApiPage[] {
  const pages: ApiPage[] = [
    {
  ...
  for (const symbol of Object.values(manifest.symbols)) {
    if (!PAGE_KINDS.has(symbol.kind)) continue;
    if (symbol.kind === "module" && symbol.route === manifest.baseRoute) continue;
    pages.push({
      route: symbol.route,
...
export function renderApiDocs(manifest: ApiManifest): ApiGenerationResult {
  return {
    manifest,
