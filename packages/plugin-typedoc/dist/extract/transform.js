...
function examplesFrom(reflection) {
    const tags = reflection.comment?.blockTags?.filter((entry) => entry.tag.replace(/^@/, "") === "example") ?? [];
    if (!tags.length)
        return undefined;
    return tags
...
function shouldInclude(reflection, ctx) {
    if (!ctx.includePrivate && reflection.flags.isPrivate)
        return false;
...
function createSymbolRecord(reflection, ctx, parent) {
    if (!shouldInclude(reflection, ctx))
        return undefined;
    const kind = kindName(reflection.kind);
    if (!kind || kind === "parameter" || kind === "enum-member")
        return undefined;
    ...
    const symbol = {
        ...
        examples: examplesFrom(reflection) ??
...
function walkDeclaration(reflection, ctx, manifest, parent) {
    ...
    if (symbol && PAGE_KINDS.has(symbol.kind)) {
        ...
        if (symbol.kind === "module")
            manifest.modules.push(symbol.id);
    }
    if (pageContainer) {
...
function buildTree(manifest) {
    ...
    const nodes = moduleSymbols.map((module) => ({
        ...
        items: Object.values(manifest.symbols)
            .filter((symbol) => symbol.id !== module.id &&
