...
function samplePath(path: string, operation: OpenApiOperation): string {
...
function sampleQuery(operation: OpenApiOperation): string {
...
function serverUrl(manifest: OpenApiManifest): string {
  return manifest.servers[0]?.url?.replace(/\/$/, "") ?? "https://api.example.com";
}

export function renderCurlExample(manifest: OpenApiManifest, operation: OpenApiOperation): string {
...
export function schemaExample(schema: OpenApiSchema): unknown {
