...
function renderParameterTable(params: OpenApiParameter[], route: string): string {
  if (!params.length) return "_None._\n";
  const rows = params.map(
    (param) =>
      `| \`${param.name}\` | ${param.in} | ${param.required ? "Yes" : "No"} | \`${param.schema}\` | ${param.example !== undefined ? `\`${JSON.stringify(param.example)}\`` : "—"} | ${param.description ?? ""} |`,
  );
  return [
...
function renderSecurity(manifest: OpenApiManifest, operation: OpenApiOperation): string {
  if (!operation.security.length) return "_No authentication required._\n";
  const lines = operation.security.map((name) => {
    ...
    if (scheme.scheme) details.push(scheme.scheme);
    if (scheme.in && scheme.name) details.push(`${scheme.in}: ${scheme.name}`);
    return `- **${name}** (${details.join(", ")})${scheme.description ? ` — ${scheme.description}` : ""}`;
  });
  return `${lines.join("\n")}\n`;
}

function renderOperationPage(manifest: OpenApiManifest, operation: OpenApiOperation): string {
  ...
  const responseRows = operation.responses.map((response) => {
    const schemaLink =
      response.schema && manifest.schemas[response.schema]
        ? `[${response.schema}](${relativeHref(operation.route, manifest.schemas[response.schema].route)})`
        : response.schema
          ? `\`${response.schema}\``
          : "—";
    const example =
      response.example !== undefined ? `\`${JSON.stringify(response.example)}\`` : "—";
...
function renderTagPage(manifest: OpenApiManifest, tagSlug: string): string {
  const tag = manifest.tags.find((entry) => entry.slug === tagSlug) ?? {
    name: tagSlug === "default" ? "Other" : tagSlug,
    slug: tagSlug,
    description: undefined,
  };
  const operations = Object.values(manifest.operations).filter((operation) =>
    tagSlug === "default" ? operation.tags.includes("default") : operation.tags.includes(tag.name),
  );
...
function renderSchemasIndex(manifest: OpenApiManifest): string {
...
function routeToRelativePath(baseRoute: string, route: string, outputDir: string): string {
  ...
  if (!suffix) return `${outputDir}/index.md`;
  return `${outputDir}/${suffix}.md`;
}

export function renderOpenApiPages(manifest: OpenApiManifest): ApiPage[] {
  const pages: ApiPage[] = [
    {
  ...
  for (const tag of manifest.tags) {
    pages.push({
      ...
      relativePath: routeToRelativePath(
        manifest.baseRoute,
        joinRoute(manifest.baseRoute, "tags", tag.slug),
        manifest.outputDir,
      ),
  ...
  if (Object.values(manifest.operations).some((operation) => operation.tags.includes("default"))) {
    pages.push({
      ...
      relativePath: routeToRelativePath(
        manifest.baseRoute,
        joinRoute(manifest.baseRoute, "tags", "default"),
        manifest.outputDir,
      ),
...
export function renderOpenApiDocs(manifest: OpenApiManifest): OpenApiGenerationResult {
  return {
    manifest,
