...
import { generateOpenApiDocs, writeGeneratedPages, writeStructuredManifest, } from "./extract/generate.js";
...
export function openapiPlugin(options) {
    ...
    return {
        ...
        async configResolved(config) {
            ...
            config.logger.info(`openapi: generated docs for ${typeof options.input === "string" ? options.input : options.input.url} under ${resultRoute(options)}${explorer.enabled ? " (explorer adapter registered)" : ""}`, { timestamp: true });
        },
    };
}
function resultRoute(options) {
