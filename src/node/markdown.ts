...
export async function renderMarkdown(
  ...
  const headings = extractHeadingsFromContent(processed).filter((heading) => heading.level >= 2 && heading.level <= 3)
  const title =
  ...
  md.renderer.rules.pp_code_group = (tokens, idx, _rendererOptions, env): string => {
    ...
    const tabs = fences
      .map((fence, index) => {
        ...
        return `<input type="radio" name="${groupId}" id="${inputId}" class="pp-code-group-input"${checked}>` +
