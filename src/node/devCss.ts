...
async function warmClientModuleGraph(server: ViteDevServer, entryMod: ModuleNode): Promise<void> {
  const seen = new Set<ModuleNode>();
...
export async function collectDevStylesheetHrefs(server: ViteDevServer): Promise<string[]> {
  ...
  const entryMod =
    server.moduleGraph.getModuleById(entryId) ?? (await server.moduleGraph.getModuleByUrl(entryId));
