...
export function absoluteUrl(site, route) {
    return canonicalUrl({ url: site.site.url, base: site.site.base, route });
}
export function renderHeadTag(tag) {
...
export function renderStylesheetLinks(hrefs) {
    return hrefs
...
function renderProductionStylesheetLinks(mainCss, base) {
    return mainCss
        .map((c) => {
...
export async function injectDevPageDocument(html, opts) {
    ...
    const devCssTags = devStylesheets?.length && !devStylesheets.every((href) => html.includes(href))
    ...
    if (/<html\b/i.test(out)) {
        out = out.replace(/<html\b([^>]*)>/i, (_match, attrs) => {
    ...
    if (out.includes('__PREACTPRESS_ROUTE__')) {
        out = out.replace(/<script>window\.__PREACTPRESS_ROUTE__=[^<]*<\/script>/, routeScript);
    }
    else {
        out = out.replace(/<body\b[^>]*>/i, (match) => `${match}\n    ${routeScript}`);
    }
    out = out.replace(/<div\s+id=["']app["'][^>]*>[\s\S]*?<\/div>/i, `<div id="app">${body}</div>`);
    return out;
}
