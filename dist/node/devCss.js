...
export function isCssModuleId(id) {
    return /\.css(?:\?|$)/.test(id);
}
/** Collect stylesheet URLs reachable from the client entry module graph. */
export function collectCssUrlsFromModule(entryMod) {
    const hrefs = new Set();
    const seen = new Set();
    const stack = [entryMod];
...
async function resolveClientEntryId(server) {
    const resolved = await server.pluginContainer.resolveId(CLIENT_ALIAS, normalizePath(`${server.config.root}/index.html`));
    return resolved?.id;
}
/** Transform client modules so Vite registers CSS dependencies on the module graph. */
async function warmClientModuleGraph(server, entryMod) {
    const seen = new Set();
    const queue = [entryMod];
...
export async function collectDevStylesheetHrefs(server) {
    const entryId = await resolveClientEntryId(server);
    if (!entryId)
        return [];
    await server.transformRequest(entryId);
    const entryMod = server.moduleGraph.getModuleById(entryId) ??
...
//# sourceMappingURL=devCss.js.map