...
The Hono-style starter intentionally keeps a focused guide subset so the custom theme stays easy to scan. Use the `docs` template as the canonical source for advanced PreactPress reference pages such as default theme options, dynamic routes, build hooks, and custom theme APIs.
